...
public class PlayerCommand {
	...
	private static void modPlayerAttribute(final CommandSender sender, final String[] args,
		...
		if (add) {
			if(attribute.getRequiredAttribute()!=null){
				PlayerAttribute ra = plugin.getObjectManager().getAttribute(attribute.getRequiredAttribute());
				if(ra!=null){
					if(!cp.getPlayerAttributes().contains(ra)){
						CrimsonIngotUtil.sendCrimsonMessage(sender,
								"That player cannot learn that Attribute.");
						return;
					}
				}
			}
	...
	private static void sendHelpText(CommandSender sender){
	...
	private static void modPlayerTitle(final CommandSender sender, final String[] args, final CrimsonIngot plugin,
		...
		if (title != "") {
			...
			CrimsonIngotUtil.sendCrimsonMessage(sender, 
					"Player " + target.getName() + "'s title has been reset");
		}
	}
	...
	public static boolean perform(final CommandSender sender, final Command cmd, final String label,
		...
		} else CrimsonIngotUtil.sendCrimsonMessage(sender, CrimsonIngotConstants.noPerm);
